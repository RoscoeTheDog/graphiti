...
class GraphitiClient:
    ...
    def _handle_connection_error(self, operation: str) -> None:
        ...
        if not self._check_daemon_enabled():
            ...
            print(f"   Daemon not running. Run:", file=sys.stderr)
            print(f"     graphiti-mcp daemon install", file=sys.stderr)
            print(f"", file=sys.stderr)
            print(f"   Or enable in config: {self.config_path}", file=sys.stderr)
            ...
            print(f"      • Port 8321 may be in use by another process", file=sys.stderr)
            print(f"      • Bootstrap service not running", file=sys.stderr)
            print(f"", file=sys.stderr)
            print(f"   Check bootstrap service status:", file=sys.stderr)
